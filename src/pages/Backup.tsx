...
import { initDatabase, createFullBackup, restoreFromBackup, type BackupData } from "@/lib/db-service";
...
const Backup = () => {
    ...
    try {
      ...
      if (isInIframe) {
        toast.error(
          "Acceso restringido en iframe", 
          { 
            description: "No se puede acceder al sistema de archivos dentro de un iframe. Por favor, abre la aplicación en una ventana completa del navegador." 
          }
        );
        return;
      }
  ...
  // Alternative backup method for browsers without File System Access API
  const performSimpleBackup = async () => {
    setIsExporting(true);
    ...
    try {
      ...
      toast.error(
        "Error al crear el respaldo", 
        { description: `No se pudo crear el archivo de respaldo: ${(error as Error).message}` }
      );
    } finally {
      setIsExporting(false);
    }
  ...
  const handleFileImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (confirm("¿Está seguro de importar este respaldo? Se sobrescribirán todos los datos actuales.")) {
    ...
    <div className="container max-w-4xl p-6 space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <Button
            onClick={isFileSystemAccessSupported() ? performBackup : performSimpleBackup}
            className="bg-gradient-to-r from-chile-blue to-chile-blue text-white"
            disabled={isExporting}
          >
            <Save className="mr-2" />
            {isExporting ? "Exportando..." : "Hacer respaldo"}
          </Button>
          <Button
      ...
      {!isFileSystemAccessSupported() && (
        <Alert className="mb-4">
          <AlertTriangle className="h-4 w-4" />
          <AlertDescription>
            Tu navegador no soporta la API de acceso al sistema de archivos avanzada. 
            Usaremos el método de descarga básico para los respaldos.
          </AlertDescription>
        </Alert>
      )}
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <Button 
                ...
                variant="outline" 
                disabled={!isFileSystemAccessSupported() || isInIframe}
              >
        ...
        <CardContent className="space-y-4">
          ...
          {isInIframe && (
            <div className="flex flex-col gap-3 p-4 bg-yellow-50 border border-yellow-200 rounded-md">
              ...
              <p className="text-sm text-yellow-700">
                Sin embargo, hemos implementado un método alternativo para exportar respaldos que funcionará en este navegador.
              </p>
            </div>
          )}
        </CardContent>
