...
import { List, Trash, Edit, Save, RefreshCw, AlertTriangle, Package, Plus, Minus } from "lucide-react";
...
const Ingredients: React.FC = () => {
  ...
  const [lowStockThreshold, setLowStockThreshold] = useState(200); // 200g default threshold
  const [criticalStockThreshold, setCriticalStockThreshold] = useState(100); // 100g default critical threshold
  ...
  // Get stock warning count
  const getLowStockCount = () => {
    return ingredients.filter(ing => ing.stock <= lowStockThreshold).length;
  };
  
  // Get critical stock warning count
  const getCriticalStockCount = () => {
    return ingredients.filter(ing => ing.stock <= criticalStockThreshold).length;
  };
  ...
  const getStockLevelColor = (stockValue: number) => {
    if (stockValue <= criticalStockThreshold) return "bg-red-600";
    if (stockValue <= lowStockThreshold) return "bg-yellow-500";
    return "bg-green-500";
  };
  
  // Function to determine stock level text color
  const getStockTextColor = (stockValue: number) => {
    if (stockValue <= criticalStockThreshold) return "text-red-500";
    if (stockValue <= lowStockThreshold) return "text-yellow-500";
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <div className="container mx-auto p-4 relative min-h-screen">
        ...
        {(getLowStockCount() > 0) && (
          <Card className="bg-[#111111] border-red-900 mb-6">
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                <AlertTriangle className="text-red-500" />
                <div>
                  <h3 className="font-semibold text-red-500">Alerta de stock</h3>
                  <p className="text-sm text-gray-300">
                    {getLowStockCount()} ingredientes con stock bajo
                    {getCriticalStockCount() > 0 && (
                      <span>, incluyendo <span className="text-red-500 font-bold">{getCriticalStockCount()} críticos</span></span>
                    )}
                  </p>
                </div>
              </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-zinc-800">
                        ...
                        <th className="py-2 text-white">Estado</th>
                      ...
                      {ingredients.map(ingredient =>
                        editId === ingredient.id ? (
                          ...
                          <tr key={ingredient.id} className="border-b border-zinc-800 hover:bg-[#252525] text-white">
                            ...
                            <td className={getStockTextColor(ingredient.stock)}>{ingredient.stock || 0}</td>
                            <td>
                              <div className="flex items-center gap-2">
                                <Progress 
                                  value={ingredient.stock} 
                                  max={1000} 
                                  className="h-2 w-16 bg-zinc-800"
                                >
                                  <div 
                                    className={`h-full ${getStockLevelColor(ingredient.stock)}`} 
                                    style={{ width: `${Math.min(100, (ingredient.stock / 1000) * 100)}%` }} 
                                  />
                                </Progress>
                                
                                {ingredient.stock <= criticalStockThreshold && (
                                  <Badge variant="destructive" className="text-xs">Crítico</Badge>
                                )}
                                {ingredient.stock > criticalStockThreshold && ingredient.stock <= lowStockThreshold && (
                                  <Badge variant="outline" className="text-xs bg-yellow-900/30 text-yellow-500 border-yellow-700">Bajo</Badge>
                                )}
                              </div>
                            </td>
