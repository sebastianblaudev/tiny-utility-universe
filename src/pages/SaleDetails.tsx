...
import SIIConfigDialog from '@/components/sales/SIIConfigDialog';

const SaleDetails = () => {
  const [searchParams] = useSearchParams();
  const saleId = searchParams.get('id');
  const navigate = useNavigate();
  const [showSIIConfig, setShowSIIConfig] = useState(false);
  ...
  return (
    <Layout>
      <div className="container mx-auto p-6">
        ...
        <SIIConfigDialog 
          isOpen={showSIIConfig}
          onClose={() => setShowSIIConfig(false)}
        />
      </div>
    </Layout>
  );
