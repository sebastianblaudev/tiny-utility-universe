...
const Estadisticas = () => {
  ...
  const calcularTotalEfectivoMixto = () => {
    if (!salesData) return 0;
    let total = 0;
    
    salesData.forEach(sale => {
      if (sale.payment_method === 'mixed' && sale.sale_payment_methods) {
        const cashAmount = sale.sale_payment_methods
          .filter(pm => pm.payment_method === 'cash')
          .reduce((sum, pm) => sum + pm.amount, 0);
        total += cashAmount;
  ...
  const calcularTotalTarjetaMixto = () => {
    if (!salesData) return 0;
    let total = 0;
  ...
  const calcularTotalTransferenciaMixto = () => {
    if (!salesData) return 0;
    let total = 0;
  ...
  const calcularTotalPagosMixtos = () => {
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <main className="flex-1 overflow-y-auto p-5">
          <div className="container mx-auto">
            ...
            {/* Nuevo badge para Pagos Mixtos */}
            {calcularTotalPagosMixtos() > 0 && (
              <div className="mb-6 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-medium text-blue-900">
                    Total: {formatCurrency(calcularTotalPagosMixtos())} ({calcularCantidadPagosMixtos()} ventas)
                  </span>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-center">
                  <div className="bg-white p-2 rounded border border-blue-100">
                    <p className="text-xs text-blue-600">Efectivo (mixto)</p>
                    <p className="font-bold text-blue-900">{formatCurrency(calcularTotalEfectivoMixto())}</p>
                  </div>
                  <div className="bg-white p-2 rounded border border-blue-100">
                    <p className="text-xs text-blue-600">Tarjeta (mixto)</p>
                    <p className="font-bold text-blue-900">{formatCurrency(calcularTotalTarjetaMixto())}</p>
                  </div>
                  <div className="bg-white p-2 rounded border border-blue-100">
                    <p className="text-xs text-blue-600">Transferencia (mixto)</p>
                    <p className="font-bold text-blue-900">{formatCurrency(calcularTotalTransferenciaMixto())}</p>
                  </div>
                </div>
              </div>
            )}
