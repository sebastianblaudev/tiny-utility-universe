...
import CashDrawerBalance from '@/components/stats/CashDrawerBalance';
...
const Estadisticas = () => {
  ...
  const { 
    ...
    queryFn: async () => {
      ...
      try {
        ...
        if (!selectedTurno) {
          if (selectedPeriod === "custom" && customDateRange && customDateRange.from) {
  ...
  const generateChartData = () => {
    ...
    if (selectedPeriod === "hoy") {
      ...
      formatString = 'dd/MM';
      groupBy = 'day';
    } else if (selectedPeriod === "custom" && customDateRange && customDateRange.from) {
          ...
          <div className="container mx-auto">
            ...
            <Card className="mb-6">
              <CardHeader className="pb-3">
                ...
                <CardDescription>
                  Selecciona un rango de fechas para filtrar las ventas de manera específica
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="flex flex-col sm:flex-row gap-4 items-end">
                    ...
                    <Popover open={showDatePicker} onOpenChange={setShowDatePicker}>
                      <PopoverTrigger asChild>
                        <Button
                          ...
                          className={cn(
                            ...
                            !(customDateRange && customDateRange.from) && "text-muted-foreground"
                          )}
                        >
                      ...
                      <PopoverContent className="w-auto p-0" align="start">
                        <Calendar
                          initialFocus
                          mode="range"
                          defaultMonth={customDateRange && customDateRange.from ? customDateRange.from : undefined}
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      disabled={!(customDateRange && (customDateRange.from || customDateRange.to))}
                    >
                      Limpiar
                    </Button>
                ...
                {/* Mostrar el resumen del filtro aplicado */}
                {selectedPeriod === "custom" && customDateRange && customDateRange.from && (
                  <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-medium">
                        Ventas desde {format(customDateRange.from, "dd/MM/yyyy", { locale: es })}
                        {customDateRange.to && (
                          <> hasta {format(customDateRange.to, "dd/MM/yyyy", { locale: es })}</>
                        )}
                      </span>
                    </div>
                    <p className="text-xs text-blue-700 mt-1">
                      Mostrando todas las ventas en el período seleccionado
                    </p>
                  </div>
                )}
              </CardContent>
