...
const deleteSale = async (saleId: string, tenantId: string): Promise<DeleteSaleResult> => {
  try {
    ...
    // Then delete the sale with tenant validation
...
const HistorialVentas2 = () => {
  ...
  const handleGenerateReceipt = (saleId: string) => {
    ...
    window.open("https://eboleta.sii.cl", "_blank");
  };
  ...
  const handleDeleteSale = async () => {
    ...
    try {
      const result = await deleteSale(saleToDelete.id, tenantId!);
