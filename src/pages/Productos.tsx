...
import { Plus, Edit, Trash, Upload, Loader2, Scale, Tag, Filter, X, Search, ChevronDown, ChevronUp, Image as ImageIcon, ArrowUpDown, FileUp, PencilLine } from 'lucide-react';
import { useToast, toast } from '@/hooks/use-toast';
import { uploadProductImage } from '@/utils/imageUploadUtils';
import { useAuth } from '@/contexts/AuthContext';
import { Switch } from '@/components/ui/switch';
import { Label } from '@/components/ui/label';
import OfflineProductsSync from '@/components/OfflineProductsSync';
...
import { PaginatedProductsView } from '@/components/products/PaginatedProductsView';
...
const Productos = () => {
  ...
  // Always use paginated view for better performance
  ...
  // Always use paginated view for better performance with 20 products per page
  ...
  useEffect(() => {
    if (currentProduct) {
      editForm.reset({
        ...
        is_by_weight: currentProduct.is_by_weight || false
      });
    }
  ...
  const fetchProducts = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('products').select('*').eq('user_id', user.id).order('name');
      if (error) throw error;
      const categoryPrefix = `__category_placeholder__`;
      const filteredProducts = data ? data.filter(product => !product.name.startsWith(categoryPrefix)) : [];
      setProducts(filteredProducts);
      console.info('Productos cargados:', filteredProducts);
    } catch (error) {
  ...
  const fetchCategories = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('products').select('category').eq('user_id', user.id).not('category', 'is', null);
      if (error) throw error;
  ...
  const handleAddCategory = async (values: CategoryFormValues) => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('products').insert([placeholderProduct]).select();
      if (error) throw error;
      toast({
        title: "Éxito",
  ...
  const handleDeleteCategory = async (category: string) => {
    ...
    try {
      ...
      const {
        error: deleteError
      } = await supabase.from('products').delete().eq('user_id', user.id).eq('name', `${categoryPrefix}${category}`);
      if (deleteError) {
  ...
  const handleAddProduct = async (values: ProductFormValues) => {
    try {
      ...
      const newProduct = {
        ...
        stock: parseFloat(values.stock || '0') || 0,
        image_url: values.image_url || null,
        category: values.category,
        is_by_weight: values.is_by_weight,
        user_id: user.id
      };
      ...
      if (error) {
        // Si hay error de código duplicado, mostrar mensaje específico
        if (error.code === '23505' && error.message.includes('products_code_tenant_unique')) {
          throw new Error(`Ya existe un producto con el código "${values.code}". Los códigos deben ser únicos.`);
        }
        throw error;
      }
      toast({
  ...
  const handleUpdateProduct = async (values: ProductFormValues) => {
    ...
    try {
      ...
      const {
        error
      } = await supabase.from('products').update(updatedProduct).eq('id', currentProduct.id).eq('user_id', user.id);
      if (error) {
        // Si hay error de código duplicado, mostrar mensaje específico
        if (error.code === '23505' && error.message.includes('products_code_tenant_unique')) {
          throw new Error(`Ya existe un producto con el código "${values.code}". Los códigos deben ser únicos.`);
  ...
  const handleDeleteProduct = async () => {
    ...
    console.log('Iniciando eliminación de producto con ID:', productToDelete.id);
    ...
    try {
      ...
      console.log('Eliminando producto directamente...');

      // Eliminar producto directamente usando el campo correcto
      const {
        error
  ...
  const ImageUploadField = ({
    ...
    return <div className="grid gap-2">
        ...
        <div className="flex flex-col items-center gap-4">
          {imageUrl ? <div className="relative w-full max-w-[200px] h-[200px] mx-auto border rounded-md overflow-hidden">
              <img src={imageUrl} alt="Preview" className="w-full h-full object-cover" />
              <Button variant="destructive" size="sm" className="absolute top-2 right-2 w-8 h-8 p-0 rounded-full" onClick={() => form.setValue('image_url', '')}>
                <Trash size={14} />
              </Button>
            </div> : <div className="w-full h-[140px] border-2 border-dashed rounded-md flex flex-col items-center justify-center cursor-pointer bg-muted/50" onClick={handleImageContainerClick}>
          ...
          <div className="w-full">
            <input id="product-image" type="file" accept="image/*" className="hidden" onChange={handleImageUpload} disabled={isUploading} ref={imageInputRef} />
            
            <Button variant="outline" className="w-full" onClick={() => imageInputRef.current?.click()} disabled={isUploading}>
              {isUploading ? <>
    ...
    try {
      ...
      switch (command.action) {
        ...
        case 'buscar':
          ...
          if (command.category) {
            const matchingCategory = categories.find(cat => cat.toLowerCase().includes(command.category?.toLowerCase() || ''));
            if (matchingCategory) {
        ...
        case 'editar':
          ...
          const productToEdit = products.find(p => p.name.toLowerCase().includes(command.name?.toLowerCase() || ''));
          if (!productToEdit) {
          ...
          const {
            error: updateError
          } = await supabase.from('products').update(updateData).eq('id', productToEdit.id).eq('user_id', user.id);
          if (updateError) {
            // Si hay error de código duplicado, mostrar mensaje específico
            if (updateError.code === '23505' && updateError.message.includes('products_code_tenant_unique')) {
              throw new Error(`Ya existe un producto con ese código. Los códigos deben ser únicos.`);
            }
            throw updateError;
          }
        ...
        case 'eliminar':
          ...
          const productToDelete = products.find(p => p.name.toLowerCase().includes(command.name?.toLowerCase() || ''));
          if (!productToDelete) {
            toast.error(`No se encontró el producto "${command.name}"`);
            return;
          }
          ...
          // Solicitar contraseña de administrador para comando de voz
          const adminPassword = prompt('Ingrese la contraseña de administrador para eliminar este producto:');
          if (!adminPassword) {
            toast.error("Se requiere contraseña de administrador para eliminar productos");
            return;
          }

          // Usar la función de seguridad con validación de admin
          const {
            data: deleteResult,
            error: deleteError
          } = await supabase.rpc('delete_product_with_admin_check', {
  ...
  return <SidebarProvider>
          ...
          <div className="flex flex-col h-full" ref={scrollTopRef}>
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex gap-2">
                    ...
                    <div className="space-y-4">
                      <Form {...categoryForm}>
                        <form onSubmit={categoryForm.handleSubmit(handleAddCategory)} className="space-y-4">
                          <FormField control={categoryForm.control} name="name" render={({
                          field
                        }) => <FormItem>
                      ...
                      <div className="border rounded-md">
                        <ScrollArea className="h-[200px] w-full rounded-md">
                          <div className="p-4 space-y-2">
                            {categories.length > 0 ? categories.map(category => <div key={category} className="flex items-center justify-between py-2 px-3 bg-secondary/20 rounded-md">
                                  <span>{category}</span>
                                  <Button variant="ghost" size="sm" onClick={() => handleDeleteCategory(category)} className="h-8 w-8 p-0">
                    ...
                    <Form {...addForm}>
                      <form onSubmit={addForm.handleSubmit(handleAddProduct)} className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <FormField control={addForm.control} name="price" render={({
                          field
                        }) => <FormItem>
                          ...
                          <FormField control={addForm.control} name="cost_price" render={({
                          field
                        }) => <FormItem>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <FormField control={addForm.control} name="code" render={({
                          field
                        }) => <FormItem>
                                <FormLabel>Código*</FormLabel>
                                <div className="flex gap-2">
                                  <FormControl>
                                    <Input placeholder="SKU o código de barras" {...field} />
                                  </FormControl>
                                  <BarcodeButton currentBarcode={field.value} onGenerate={barcode => field.onChange(barcode)} disabled={isLoading} />
                                </div>
                              </FormItem>} />
                          
                          <FormField control={addForm.control} name="stock" render={({
                          field
                        }) => <FormItem>
                                <FormLabel>Stock</FormLabel>
                                <FormControl>
                                  <Input type="number" placeholder="0" step="0.001" min="0" {...field} />
                                </FormControl>
                              </FormItem>} />
                        </div>
                        
                        <FormField control={addForm.control} name="category" render={({
                        field
                      }) => <FormItem>
                              <FormLabel>Categoría</FormLabel>
                              <Select value={field.value || "no-category"} onValueChange={value => field.onChange(value === "no-category" ? null : value)}>
                                <SelectTrigger>
                        ...
                        <FormField control={addForm.control} name="is_by_weight" render={({
                        field
                ...
                <VoiceCommandButton onCommand={handleVoiceCommand} isListening={isListening} setIsListening={setIsListening} />
                
                <Button variant="outline" size="sm" onClick={scrollToTop} className="ml-auto" title="Volver arriba">
                  <ChevronUp size={16} />
                </Button>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-4 items-start">
              <div className="w-full sm:w-auto lg:flex-1 flex gap-2">
                <div className="relative w-full">
                  ...
                  {searchQuery && <Button variant="ghost" size="sm" className="absolute right-0 top-0 h-full px-3" onClick={() => setSearchQuery('')}>
                      <X size={16} />
                    </Button>}
                </div>
              </div>
            ...
            {filteredProducts.length > 0 ? usePaginatedView ? <PaginatedProductsView onProductEdit={product => handleEditClick(product)} /> : <div className="rounded-md border">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead className="hidden lg:table-cell w-[50px]">
                        #
                      </TableHead>
                      ...
                      <TableHead className="cursor-pointer hidden sm:table-cell" onClick={() => requestSort('price')}>
                        <div className="flex items-center gap-1">
                          Precio {renderSortIcon('price')}
                        </div>
                      </TableHead>
                      <TableHead className="cursor-pointer hidden lg:table-cell" onClick={() => requestSort('cost_price')}>
                        <div className="flex items-center gap-1">
                          Costo {renderSortIcon('cost_price')}
                        </div>
                      </TableHead>
                      <TableHead className="cursor-pointer hidden md:table-cell" onClick={() => requestSort('stock')}>
                        <div className="flex items-center gap-1">
                  ...
                  <TableBody>
                    {sortedProducts.map((product, index) => <React.Fragment key={product.id}>
                        <TableRow className={expandedRows[product.id] ? "border-b-0" : ""} onClick={() => toggleRowExpansion(product.id)}>
                          <TableCell className="hidden lg:table-cell w-[50px]">
                            {index + 1}
                          </TableCell>
                          <TableCell className="font-medium">
                            <div className="flex items-center gap-2">
                              {product.image_url ? <div className="w-8 h-8 rounded overflow-hidden bg-muted">
                                  <img src={product.image_url} alt={product.name} className="w-full h-full object-cover" />
                                </div> : <div className="w-8 h-8 rounded flex items-center justify-center bg-muted">
                              ...
                              {product.is_by_weight && <div className="ml-1 px-1.5 py-0.5 bg-amber-100 text-amber-800 text-xs rounded-sm flex items-center gap-1">
                                  <Scale size={12} />
                                  <span>Por peso</span>
                                </div>}
                            </div>
                          ...
                          <TableCell className="hidden lg:table-cell">
                            <div className="w-full overflow-hidden text-ellipsis whitespace-nowrap" style={{
                        maxWidth: "150px"
                      }}>
                              {product.code || "-"}
                            </div>
                          ...
                          handleDeleteClick(product);
                              ...
                              <Button variant="ghost" size="sm" className="h-8 w-8 p-0 md:hidden" onClick={e => {
                          e.stopPropagation();
                        ...
                        {expandedRows[product.id] && <TableRow className="bg-muted/30">
                            <TableCell colSpan={8} className="py-2">
                              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 p-2">
                                ...
                                <div className="space-y-1">
                                  ...
                                  <p className="text-sm">{product.is_by_weight ? "Por peso" : "Por unidad"}</p>
                                </div>
                                
                                {product.code && /^\d{13}$/.test(product.code) && <div className="col-span-full">
                                    <h4 className="text-sm font-semibold mb-1">Código de barras:</h4>
                                    <div className="bg-white p-3 rounded-md inline-block">
                                      <svg className="h-14" jsbarcode-format="ean13" jsbarcode-value={product.code} jsbarcode-textmargin="0" jsbarcode-fontoptions="bold"></svg>
                                    </div>
                                  </div>}
                              </div>
                            </TableCell>
                ...
                </div> : <div className="border rounded-md flex flex-col items-center justify-center py-12 px-4">
                ...
                <p className="text-muted-foreground text-center mb-6 max-w-md">
                  {searchQuery || activeCategory ? "No se encontraron productos con los filtros seleccionados. Intenta con otra búsqueda o categoría." : "Aún no has agregado productos. Haz clic en el botón 'Agregar Producto' para comenzar."}
                </p>
                <Button onClick={() => {
              if (searchQuery || activeCategory) {
                ...
                setIsAddDialogOpen(true);
                  ...
                  {searchQuery || activeCategory ? "Limpiar filtros" : "Agregar primer producto"}
                </Button>
              </div>}
          </div>
        </SidebarInset>
        ...
        <DialogContent className="sm:max-w-[525px]">
          ...
          <Form {...editForm}>
            <form onSubmit={editForm.handleSubmit(handleUpdateProduct)} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <FormField control={editForm.control} name="price" render={({
                field
              }) => <FormItem>
                      <FormLabel>Precio de venta*</FormLabel>
                      <FormControl>
                ...
                <FormField control={editForm.control} name="cost_price" render={({
                field
              }) => <FormItem>
                      <FormLabel>Precio de costo</FormLabel>
                      <FormControl>
              ...
              <div className="grid grid-cols-2 gap-4">
                <FormField control={editForm.control} name="code" render={({
                field
              }) => <FormItem>
                      <FormLabel>Código*</FormLabel>
                      <div className="flex gap-2">
                ...
                <FormField control={editForm.control} name="stock" render={({
                field
              }) => <FormItem>
                      <FormLabel>Stock</FormLabel>
                      <FormControl>
              ...
              <FormField control={editForm.control} name="category" render={({
              field
            }) => <FormItem>
                    <FormLabel>Categoría</FormLabel>
                    <Select value={field.value || "no-category"} onValueChange={value => field.onChange(value === "no-category" ? null : value)}>
                ...
                <FormField control={editForm.control} name="is_by_weight" render={({
              field
            }) => <FormItem className="flex items-center gap-2 space-y-0">
                    <FormControl>
                      <Switch checked={field.value} onCheckedChange={field.onChange} />
      ...
      <UnlockDialog isOpen={isUnlockDialogOpen} onClose={() => setIsUnlockDialogOpen(false)} onUnlock={handleDeleteProduct} itemName={`Eliminar producto "${productToDelete?.name || ''}"`} />
    </SidebarProvider>;
};
