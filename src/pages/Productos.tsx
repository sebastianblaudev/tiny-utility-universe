...
const Productos = () => {
  ...
  // Fast POS Plugin for automatic pagination
  const { shouldAutoActivate } = useFastPOSPlugin();
  ...
  // Check if we should use paginated view based on product count
  useEffect(() => {
    const checkPaginatedView = async () => {
      if (products.length > 1000) {
        const shouldUse = await shouldAutoActivate(products.length);
        setUsePaginatedView(shouldUse);
      }
    };
  ...
  }, [products.length, shouldAutoActivate]);
    ...
    try {
      ...
      switch (command.action) {
        ...
        case 'eliminar':
          ...
          const { data: deleteResult, error: deleteError } = await supabase.rpc('delete_product_with_admin_check', {
  ...
  return (
    <SidebarProvider>
            ...
            {filteredProducts.length > 0 ? (
              usePaginatedView ? (
                <PaginatedProductsView 
                  onProductEdit={(product) => handleEditClick(product)}
                />
              ) : (
      ...
      <UnlockDialog
        ...
        onUnlock={handleDeleteProduct}
        itemName={`Eliminar producto "${productToDelete?.name || ''}"`}
      />
    </SidebarProvider>
  );
};
