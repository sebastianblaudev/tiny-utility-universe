...
import { registerOperation } from "@/utils/operationSync";

// Database version and name constants to match with src/lib/db.ts
const DB_VERSION = 9;
const DB_NAME = 'pizza-pos-db';

// Define the default sizes structure to avoid TypeScript errors
const initialSizes = {
  personal: 0,
  mediana: 0,
  familiar: 0
...
export const updateIngredientsStock = async (productId: string, quantity: number = 1, size?: string) => {
  try {
    console.log(`Actualizando stock para producto ID: ${productId}, cantidad: ${quantity}, tamaño: ${size || 'general'}`);
    ...
    let ingredientsToUse: ProductIngredient[] = product.ingredients || [];
    ...
    if (size && product.ingredientsBySize && product.ingredientsBySize[size] && product.ingredientsBySize[size].length > 0) {
      ...
      console.log(`Usando ingredientes específicos para tamaño ${size}:`, ingredientsToUse);
    } else {
      console.log(`Usando ingredientes generales para tamaño ${size || 'general'}:`, ingredientsToUse);
    }
    
    if (!product || !ingredientsToUse || ingredientsToUse.length === 0) {
    ...
    const updatedIngredients = ingredients.map(ingredient => {
      const productIngredient = ingredientsToUse.find(pi => pi.id === ingredient.id);
      ...
      // Asegurar que stock sea un número y realizar la resta
      const currentStock = typeof ingredient.stock === 'number' ? ingredient.stock : parseInt(ingredient.stock) || 0;
      const newStock = Math.max(0, currentStock - amountToSubtract);
      console.log(`Stock anterior: ${currentStock}g, nuevo stock: ${newStock}g`);
    ...
    if (updated) {
      try {
        ...
        registerOperation('inventario_actualizado', { 
          productId, 
          quantity,
          size
        });
...
const Products = () => {
  ...
  const [selectedProductIngredientsBySize, setSelectedProductIngredientsBySize] = React.useState<{[size: string]: ProductIngredient[]}>({});
  const [activeProductId, setActiveProductId] = React.useState<string | null>(null);
  const [activeProductSizes, setActiveProductSizes] = React.useState<string[]>(['personal', 'mediana', 'familiar']);
  ...
  const handleSizePriceChange = (size: string, value: number) => {
    if (editingProduct) {
      ...
      const currentSizes = editingProduct.sizes || { personal: 0, mediana: 0, familiar: 0 };
      ...
      // Fix for the type error on newProduct.sizes
      const currentSizes = newProduct.sizes || { personal: 0, mediana: 0, familiar: 0 };
  ...
  const handleAddProduct = async () => {
    try {
      ...
      const productData = {
        ...
        sizes: newProduct.sizes || { personal: 0, mediana: 0, familiar: 0 }
      } as Product;
  ...
  const handleSaveIngredients = async (ingredients: ProductIngredient[], ingredientsBySize?: {[size: string]: ProductIngredient[]}) => {
    try {
      const db = await initDB();
      const product = products.find((p: any) => p.id === activeProductId);
      if (!product) return;
      
      // Guardar tanto los ingredientes generales como los específicos por tamaño
      ...
      setSelectedProductIngredientsBySize({});
      
      // Registrar operación para sincronización
      registerOperation('ingredientes_actualizados', { 
        productId: product.id,
        ingredientsCount: ingredients.length,
        sizeIngredientsCount: ingredientsBySize ? Object.keys(ingredientsBySize).length : 0
      });
  ...
  const handleOpenIngredients = (product: any) => {
    ...
    const sizeKeys = product.sizes ? Object.keys(product.sizes) : ['personal', 'mediana', 'familiar'];
    
    setSelectedProductIngredients(product.ingredients || []);
    setSelectedProductIngredientsBySize(product.ingredientsBySize || {});
    setActiveProductId(product.id);
    setActiveProductSizes(sizeKeys);
  ...
  return (
    <div className="container mx-auto p-4 space-y-6 bg-black text-white min-h-screen font-sans">
      ...
      <EditProductIngredientsModal
        ...
        ingredientsBySize={selectedProductIngredientsBySize}
        availableSizes={activeProductSizes}
