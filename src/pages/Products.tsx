...
import { Pencil, Trash2, PlusCircle, Search, Tag, FileDown, FileUp, Download, Copy } from "lucide-react";
  ...
  const handleDuplicate = async (product: Product) => {
    try {
      const duplicatedProduct: Product = {
        // Don't include the ID so a new one will be generated
        name: `${product.name} (copia)`,
        category: product.category,
        unitPrice: product.unitPrice,
        // Create a new SKU for the duplicated product if it exists
      ...
      const savedProduct = await saveProduct(duplicatedProduct);
                  ...
                  {filteredProducts.map((product) => (
                    ...
                    >
                      ...
                      <td className="p-3 text-center">
                        <span className={`px-2 py-1 rounded-full text-xs font-semibold ${product.available === true ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                          {product.available === true ? 'Sí' : 'No'}
                        </span>
                      </td>
