...
const Products = () => {
  ...
  const simulateProductSale = async (productId: string) => {
    const result = await updateIngredientsStock(productId, 1);
    if (result) {
      toast({
        title: "Venta simulada",
        description: "Se ha simulado una venta y actualizado el inventario"
      });
    } else {
      toast({
        title: "No hubo cambios en el inventario",
        description: "El producto no tiene ingredientes o no se pudo actualizar el stock"
      });
    }
  };
          ...
          <Table className="bg-neutral-800 rounded-lg overflow-hidden shadow-xl border border-neutral-700">
            <TableHeader>
              <TableRow className="border-b border-neutral-700 hover:bg-neutral-700/40 transition-colors duration-300">
                ...
                <TableHead className="text-white tracking-wide">Simular</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
                ...
                >
                  ...
                  <TableCell>
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => simulateProductSale(product.id)}
