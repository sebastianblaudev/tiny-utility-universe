...
import { Plus, Edit, Trash, Save, Utensils, Barcode, X, Check, Pencil, Search, Copy } from "lucide-react";
...
const Products = () => {
  ...
  const handleSizePriceChange = (size: string, value: number) => {
    if (editingProduct) {
      ...
      const currentSizes = editingProduct.sizes || { ...initialSizes };
      ...
      // Ensure sizes exists and has the required properties with proper typing
      const currentSizes = newProduct.sizes || { ...initialSizes };
  ...
  // Add new function to duplicate product
  const handleDuplicateProduct = async (product: Product) => {
    try {
      ...
      // Create a new ID for the duplicated product
      const id = `p${Date.now()}`;
      
      // Create a copy of the product with the new ID and a "(copia)" suffix on the name
      const duplicatedProduct = {
        ...product,
        id,
        name: `${product.name} (copia)`,
        // Generate a new barcode if the original had one
        barcode: product.barcode ? generateBarcode() : null
      };
      
      // Add the duplicated product to the database
      await db.add('products', duplicatedProduct);
      
      toast("Producto duplicado exitosamente", {
        description: `Se ha creado una copia de "${product.name}"`
      });
