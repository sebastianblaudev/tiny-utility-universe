...
import { Mic, X, Plus, Minus, ShoppingCart, CreditCard, Banknote, ArrowDownToLine, Printer, ClipboardList, GridIcon, Search, LogOut, Menu, ExternalLink, Divide, UserPlus, Weight, Send, ChefHat, Smartphone } from 'lucide-react';
...
import { MobilePOSOptimizer } from '@/components/pos/MobilePOSOptimizer';
...
const POS = () => {
  ...
  const [showMobilePOSOptimizer, setShowMobilePOSOptimizer] = useState(false);
...
useEffect(() => {
  ...
  // Auto-activate mobile POS for mobile devices
  useEffect(() => {
    if (isMobile && !showFastPOS && !showMobilePOS) {
      // Auto-show mobile POS optimizer on first load for mobile devices
      const hasSeenMobilePOS = localStorage.getItem('has_seen_mobile_pos');
      if (!hasSeenMobilePOS) {
        setTimeout(() => {
          setShowMobilePOSOptimizer(true);
        }, 1000); // Show after 1 second to let the page load
      }
    }
  }, [isMobile, showFastPOS, showMobilePOS]);

  const handleActivateMobilePOS = () => {
    setShowMobilePOSOptimizer(false);
    setShowMobilePOS(true);
    localStorage.setItem('has_seen_mobile_pos', 'true');
    localStorage.setItem('prefers_mobile_pos', 'true');
  };

  const handleDismissMobilePOS = () => {
    setShowMobilePOSOptimizer(false);
    localStorage.setItem('has_seen_mobile_pos', 'true');
    localStorage.setItem('prefers_mobile_pos', 'false');
  };
  ...
  <div className="receipt-footer">
    ...
    <div className="barcode-section">
      ...
      {/* Mobile POS Optimizer */}
      {showMobilePOSOptimizer && (
        <MobilePOSOptimizer
          onActivate={handleActivateMobilePOS}
          onDismiss={handleDismissMobilePOS}
        />
      )}
      ...
      {showMobilePOS && (
        <div className="fixed inset-0 z-50 bg-background">
          <MobilePOSView />
          <Button 
            onClick={() => setShowMobilePOS(false)}
      ...
      {/* Mobile Quick Access */}
      {isMobile && !showMobilePOS && !showFastPOS && !showMobilePOSOptimizer && (
        <div className="fixed bottom-4 right-4 z-40">
          <Button
            onClick={() => setShowMobilePOS(true)}
            size="lg"
