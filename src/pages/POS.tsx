...
useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Use offline-compatible product loading with better error handling
        ...
        // Always show success message when products are loaded
        if (filteredData.length > 0) {
          const statusMessage = isOnline ? 
            `${filteredData.length} productos cargados` : 
            `${filteredData.length} productos desde cache offline`;
          
          toast.success(statusMessage, { duration: 2000 });
        } else if (!isOnline && filteredData.length === 0) {
          toast.warning("Modo Offline - Sin productos en cache", {
            description: "Conecte a internet para cargar productos"
          });
        }
      } catch (error) {
        console.error('Error al cargar productos:', error);
        toast.error("Error al cargar productos", {
          description: isOnline ? "Problema de conexión" : "Sin productos en cache offline"
        });
        
        // Set empty arrays to prevent app crashes
        setProducts([]);
        setFilteredProducts([]);
        setCategories([]);
      } finally {
    ...
    fetchProducts();
  }, [isOnline]); // Removed loadOfflineProducts dependency to prevent infinite loop
  ...
  const processSplitPayment = async () => {
    ...
    try {
      ...
      if (paymentParts.length > 1) {
        ...
        const success = await saveMixedPaymentMethods(sale.id, mixedPayments);
        if (!success) {
