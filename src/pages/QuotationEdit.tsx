...
import { getProducts, saveQuotation, getCompanyInfo, getQuotationById, type Product, type QuotationItem, type Quotation } from "@/lib/db-service";
import { PlusCircle, MinusCircle, ArrowLeft, Copy, Edit } from "lucide-react";
...
const QuotationEdit = () => {
  ...
  useEffect(() => {
    // Load products, company info and quotation data
    const loadData = async () => {
      try {
        setLoadingData(true);
        
        // Load products and company info
        const [productList, company] = await Promise.all([
          getProducts(),
          getCompanyInfo()
        ]);
        ...
        const isDuplicateParam = urlParams.get('duplicate') === 'true';
        const isEditParam = urlParams.get('edit') === 'true';
        
        setIsDuplicate(isDuplicateParam);
        setIsEdit(isEditParam || (!isDuplicateParam && !!id)); // Convert to boolean with !!

        // Load existing quotation if editing or duplicating
        if (id) {
          const quotation = await getQuotationById(id);
          if (quotation) {
            setClientName(quotation.clientName);
            setClientRut(quotation.clientRut || "");
  ...
  // Generate quotation ID for duplicates or create revision for edits
  const generateQuotationId = () => {
    ...
    if (isDuplicate) {
      // Get today's count for duplicates
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Generate quotation ID based on mode
      ...
      let successMessage = "Cotización guardada exitosamente";
      if (isDuplicate) {
        successMessage = "Cotización duplicada exitosamente";
      } else if (isEdit) {
  ...
  const getPageTitle = () => {
    if (isDuplicate) return "Duplicar Cotización";
    if (isEdit) return `Editar Cotización ${id}`;
    return "Nueva Cotización";
  };

  const getPageIcon = () => {
    if (isDuplicate) return <Copy className="h-8 w-8 inline mr-2" />;
    if (isEdit) return <Edit className="h-8 w-8 inline mr-2" />;
    return null;
  };
  ...
  const getButtonText = () => {
    ...
    if (isDuplicate) return "Duplicar Cotización";
    if (isEdit) return "Actualizar Cotización";
