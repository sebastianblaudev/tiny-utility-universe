...
const PrintReceiptModal = ({ isOpen, onClose, saleId, autoPrint = false, receiptData, isComanda = false }: PrintReceiptModalProps) => {
  ...
  useEffect(() => {
    const loadTicketData = async () => {
      if (!isOpen || (!saleId && !receiptData)) return;
      
      setLoading(true);
      setError(null);
      
      try {
        if (receiptData && isComanda) {
        ...
        const receiptDataFromDb = await getReceiptData(saleId, tenantId);
        
        if (receiptDataFromDb) {
          console.log("Received receipt data:", receiptDataFromDb);
          
          if (receiptDataFromDb.items && receiptDataFromDb.items.length > 0) {
            receiptDataFromDb.items = receiptDataFromDb.items.map((item) => ({
              ...item,
              notes: item.notes || ''
            }));
          } else {
          ...
          receiptDataFromDb.businessInfo = businessInfo;
          setTicketData(receiptDataFromDb);
        } else {
    ...
    loadTicketData();
  }, [isOpen, saleId, tenantId, businessInfo, receiptData, isComanda]);
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={isComanda ? "Comanda de Cocina" : "Recibo de Compra"}
      size="md"
    >
      {loading ? (
        ...
        <div className="bg-white p-1 print:p-0">
          ...
          {/* Receipt Display */}
          {isComanda ? (
            <ComandaTicket receiptData={receiptData || ticketData} />
          ) : viewMode === '80mm' ? (
