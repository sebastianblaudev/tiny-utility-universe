...
export function PaymentModal({ 
  ...
  const handleApplyPromotion = (discount: number, promotion: any) => {
  ...
  const taxAmount = calculateTaxAmount(discountedSubtotal);
  const subtotal = discountedSubtotal;
  const tipAmount = calculateTipAmount();
  
  // Calculate final total including tax if enabled and discounts
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const newOrder = {
        ...
        status: 'completed' as const, // Explicitly type this as a literal to match the Order interface
        createdAt: new Date(),
      ...
      // Record promotion usage if one was applied
      if (appliedPromotion) {
        try {
          const promoTx = db.transaction('promotions', 'readwrite');
          const promoStore = promoTx.objectStore('promotions');
          const promotion = await promoStore.get(appliedPromotion.id);
          
          if (promotion) {
            promotion.usageCount = (promotion.usageCount || 0) + 1;
            await promoStore.put(promotion);
            console.log(`Recorded usage for promotion ${promotion.name}, new count: ${promotion.usageCount}`);
          }
        } catch (err) {
