...
const Sidebar = () => {
  ...
  const handleLockToggle = async (item: SidebarItem, event: React.MouseEvent) => {
    ...
    if (isCurrentlyLocked) {
      ...
      try {
        const result = await toggleMenuItemLock(item.route, true);
        if (result) {
  ...
  const handleSuccessfulUnlock = async () => {
    ...
    try {
      const result = await toggleMenuItemLock(unlockingItem.route, false);
      if (result) {
        await refreshLockedItems();
        toast.success(`${unlockingItem.title} desbloqueado`, {
