      ...
      try {
        ...
        if (otherTransactions) {
          otherTransactions.forEach(trans => {
            ...
            const hasSaleId = Boolean((trans as any).venta_id);
            const desc = (trans.descripcion || '').toLowerCase();
            if (trans.tipo === 'egreso') {
              totalEgresos += monto;
            } else if (trans.tipo === 'ingreso') {
              // Contar solo ingresos NO vinculados a ventas ni de apertura
              if (!hasSaleId && !desc.includes('apertur')) {
                otherIngresos += monto;
              }
            }
          });
