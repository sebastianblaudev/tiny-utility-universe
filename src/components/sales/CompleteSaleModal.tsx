...
const CompleteSaleModal: React.FC<CompleteSaleModalProps> = ({
  ...
  // Function to add mixed payment transactions to turno
  const addMixedPaymentTransactionsToTurno = async (saleId: string, methods: MixedPaymentType[]) => {
    if (!tenantId) return;

    try {
      // Get active turno to add individual transactions for each payment method
      const { getTurnoActivo } = await import('@/utils/turnosUtils');
      const activeTurno = await getTurnoActivo(tenantId);
      
      if (activeTurno) {
        // Add individual transactions for each payment method
        for (const method of methods) {
          await agregarTransaccionTurno({
    ...
    try {
      ...
      if (isMixedPayment && mixedPaymentMethods && saleId) {
        ...
        if (!success) {
          ...
          // Add individual transactions to turno for each payment method
          await addMixedPaymentTransactionsToTurno(saleId, mixedPaymentMethods);
        }
      }
