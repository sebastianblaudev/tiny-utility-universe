...
import SIIStatusChip from './SIIStatusChip';
import { useAuth } from '@/contexts/AuthContext';
import { toast } from 'sonner';
import { retrySendSaleToSII, verifySIIStatus } from '@/utils/siiChileUtils';
...
const SaleProductsViewer = ({ saleId }: SaleProductsViewerProps) => {
  ...
  useEffect(() => {
    const fetchSaleData = async () => {
      ...
      try {
        ...
        // Check SII status
        const siiResult = await verifySIIStatus(saleId);
  ...
  const handleRetrySII = async () => {
    if (!tenantId) {
      toast.error('Error: No se pudo determinar el ID del negocio');
      return;
    }
    
    setSiiStatus(prev => ({ ...prev, retrying: true }));
    
    try {
      const result = await retrySendSaleToSII(saleId, tenantId);
      ...
      if (result.success) {
        ...
        setSiiStatus(prev => ({ ...prev, retrying: false }));
        toast.error(result.message);
      }
    } catch (error) {
      console.error('Error retrying SII send:', error);
      setSiiStatus(prev => ({ ...prev, retrying: false }));
          ...
          <div>
            {!siiStatus.sent && (
              <Button 
                ...
                size="sm" 
                className="gap-1" 
                onClick={handleRetrySII}
                disabled={siiStatus.retrying}
              >
                {siiStatus.retrying ? (
                  <>
