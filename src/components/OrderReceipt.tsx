...
export const OrderReceipt: React.FC<OrderReceiptProps> = ({ order, receiptType }) => {
  ...
  // Split header and footer text by newline characters
        ...
        <>
          <div className="text-center mb-4">
            ...
            {headerLines.map((line, index) => (
              <p key={`header-${index}`} className={index === 0 ? "text-xl font-bold mb-1" : "text-sm text-gray-600"}>
                {line}
              </p>
            ))}
          </div>
          ...
          <div className="mb-4">
            ...
            {order.customerName && (
              <div className="text-sm">Cliente: {order.customerName}</div>
            )}
          </div>
        </>
      ) : (
