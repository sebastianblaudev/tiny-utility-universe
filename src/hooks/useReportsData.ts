...
// Nueva función para obtener los registros de cocina
export const useKitchenRecords = () => {
  const fetchKitchenRecords = async () => {
    try {
      const db = await openDB(DB_NAME, DB_VERSION);
      const records = await db.getAll('kitchenRecords');
      return records;
    } catch (error) {
  ...
  return useQuery({
    queryKey: ['kitchen-records'],
    queryFn: fetchKitchenRecords,
