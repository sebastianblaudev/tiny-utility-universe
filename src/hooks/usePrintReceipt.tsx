...
import { usePrintReceiptQZ } from './usePrintReceiptQZ';
...
export const usePrintReceipt = () => {
  ...
  // Integrar QZ Tray
  const { printReceiptQZ, isQZConnected } = usePrintReceiptQZ();
  ...
  const printReceiptDirectly = async (saleId: string, paperWidth?: '80mm' | '58mm') => {
    // Intentar primero con QZ Tray, luego fallback a navegador
    const success = await printReceiptQZ(saleId, paperWidth);
    if (success) {
      return true;
    }
