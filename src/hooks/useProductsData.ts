...
// Actualizamos la versión de la base de datos para que coincida con la versión actual
const DB_VERSION = 9;
...
export function useProductsData() {
      ...
      try {
        ...
        normalizedProducts.forEach((prod: Product) => {
          ...
          if (grouped[catId]) {
            ...
            if (isPizza && (!prod.sizes || Object.keys(prod.sizes).length === 0)) {
  ...
  const addCustomSizeToProduct = async (productId: string, sizeName: string, price: number) => {
    try {
      ...
      registerOperation('producto_modificado', { 
        productId, 
        action: 'add_size', 
        sizeName, 
        price 
  ...
  const renameSizeForProduct = async (productId: string, oldSizeName: string, newSizeName: string) => {
    try {
      ...
      // Update ingredients by size if they exist
      if (product.ingredientsBySize && product.ingredientsBySize[oldSizeName]) {
        const ingredientsForSize = product.ingredientsBySize[oldSizeName];
        
        if (!product.ingredientsBySize) {
          product.ingredientsBySize = {};
        }
        
        // Copy ingredients from old size name to new size name
        product.ingredientsBySize[newSizeName.toLowerCase()] = ingredientsForSize;
        
        // Delete the old size ingredients
        delete product.ingredientsBySize[oldSizeName];
      }
