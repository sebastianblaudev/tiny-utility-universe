...
export const getBusinessInfoForReceipt = async (forceRefresh = false): Promise<BusinessInfo> => {
  ...
  try {
    console.log('Obteniendo información actualizada del negocio...', { forceRefresh });
    ...
    if (!forceRefresh) {
      ...
      if (cachedData) {
        ...
        if (parsedData.logo) {
          console.log('Logo encontrado en caché');
        } else {
          console.log('No hay logo en caché');
        }
        return parsedData;
      }
    }
    ...
    console.log('Metadata recuperada del usuario:', {
      ...metadata,
      businessLogo: metadata.businessLogo ? 'PRESENTE' : 'NO ENCONTRADO'
    });
    ...
    const businessInfo: BusinessInfo = {
      ...
      logo: metadata.businessLogo // Include logo data from user metadata
    };
    
    console.log('Información de negocio construida:', {
      ...businessInfo,
      logo: businessInfo.logo ? 'PRESENTE' : 'NO ENCONTRADO'
    });
    
    localStorage.setItem(BUSINESS_INFO_CACHE_KEY, JSON.stringify(businessInfo));
    console.log('Información de negocio actualizada en caché');
