...
// Función mejorada para manejar correctamente los pagos mixtos
export const getSalesByPaymentMethod = async (tenantId: string, startDate?: string, endDate?: string) => {
  if (!tenantId) {
    console.error("No tenant ID provided for getSalesByPaymentMethod");
    return {};
  }

  try {
    console.log(`Getting sales by payment method for tenant: ${tenantId}`);
    console.log(`Date range: ${startDate} to ${endDate}`);
    
    // Usar la función de base de datos que distribuye correctamente los pagos mixtos
    ...
    if (!data || data.length === 0) {
      console.log("No sales data found for the given parameters");
      return {};
    }
    ...
    data.forEach((item: any) => {
      ...
      console.log(`Processing payment method: ${method}, total: ${total}, count: ${count}`);
    ...
    console.log('Final payment totals with mixed payments properly distributed:', paymentTotals);
    
    // Verificar que los totales sean consistentes
    const totalAmount = Object.values(paymentTotals).reduce((sum, method) => sum + method.total, 0);
    console.log(`Total amount across all payment methods: ${totalAmount}`);
    
    return paymentTotals;
  } catch (error) {
    console.error("Exception in getSalesByPaymentMethod:", error);
    return {};
  }
};
