-- Fix the type mismatch in get_sales_by_payment_method function
...
-- Also fix the turno version to ensure consistency
...
-- Test that we have some sample data to work with
...
AND s.id NOT IN (SELECT sale_id FROM sale_payment_methods)
LIMIT 1;
...
FROM sales s 
WHERE s.payment_method = 'mixed' 
AND s.id NOT IN (SELECT sale_id FROM sale_payment_methods WHERE payment_method = 'card')
LIMIT 1;